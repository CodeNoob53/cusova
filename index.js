...
`,b="/cusova/",w=new K(b,{hash:!1}),ae={home:G,favorites:Y},A={"partials/hero.html":X,"partials/filters.html":ee,"partials/quote.html":te,"partials/sidebar-image.html":ne,"partials/info-card.html":ie};function se(e){const t=/<load\s+src="([^"]+)"\s*\/>/g;return e.replace(t,(n,i)=>A[i]?A[i]:(console.warn(`Partial not found: ${i}`),`<!-- Failed to load ${i} -->`))}function re(e){return e=e.replace(/src="\/img\//g,`src="${b}img/`),e=e.replace(/srcset="\s*\/img\//g,`srcset="${b}img/`),e=e.replace(/srcset="([^"]*)"/g,(t,n)=>`srcset="${n.replace(/\/img\//g,`${b}img/`)}"`),e}async function I(e){const t=document.getElementById("app");if(!t)return;const n=ae[e];if(!n){t.innerHTML="<h1>Page not found</h1>";return}let i=se(n);i=re(i),t.innerHTML=i}const oe="https://your-energy.b.goit.study/api",p=Z.create({baseURL:oe}),ce=async()=>{const{data:e}=await p.get("/quote");return e},le=async({filter:e,page:t=1,limit:n=12})=>{const{data:i}=await p.get("/filters",{params:{filter:e,page:t,limit:n}});return i},de=async({bodypart:e,muscles:t,equipment:n,keyword:i,page:a=1,limit:s=10})=>{const r={page:a,limit:s};e&&(r.bodypart=e),t&&(r.muscles=t),n&&(r.equipment=n),i&&(r.keyword=i);const{data:c}=await p.get("/exercises",{params:r});return c},L=async e=>{const{data:t}=await p.get(`/exercises/${e}`);return t},$=async(e,t,n,i="")=>{const a={rate:t,email:n};i&&(a.review=i);const{data:s}=await p.patch(`/exercises/${e}/rating`,a);return s},me=async e=>{const{data:t}=await p.post("/subscription",{email:e});return t},ge=`<li class="exercise-card" data-id="{{id}}">
...
`,fe=`<li class="favorite-card" data-id="{{id}}">
...
`,he={"exercise-card":ge,"category-card":ue,"favorite-card":fe,"favorites-empty":ve,pagination:pe},H={...he},y=async e=>H[e]?H[e]:(console.warn(`Template not found in bundle: ${e}`),""),C=(e,t)=>e.replace(/\{\{(\w+)\}\}/g,(n,i)=>t[i]!==void 0?t[i]:""),ye=e=>{const t=document.getElementById("quote-text"),n=document.getElementById("quote-author");t&&e.quote&&(t.textContent=e.quote),n&&e.author&&(n.textContent=e.author)},be=async(e,t)=>{const n=document.getElementById(t);if(!n)return;if(e.length===0){n.innerHTML='<p class="no-results">No exercises found. Try another filter.</p>';return}const i=await y("exercise-card"),a=e.map(s=>C(i,{id:s._id,rating:s.rating||0,ratingFormatted:s.rating?s.rating.toFixed(1):"0.0",name:s.name,burnedCalories:s.burnedCalories||0,time:s.time||0,bodyPart:s.bodyPart||"N/A",target:s.target||"N/A"})).join("");n.innerHTML=`<ul class="exercises-grid">${a}</ul>`},xe=async(e,t)=>{const n=document.getElementById(t);if(!n)return;if(e.length===0){n.innerHTML='<p class="no-results">No categories found.</p>';return}const i=await y("category-card"),a=e.map(s=>C(i,{filter:s.filter,name:s.name,imgURL:s.imgURL||""})).join("");n.innerHTML=`<ul class="categories-grid">${a}</ul>`},O=(e,t,n=9)=>{const i=document.getElementById(t);if(!i)return;let a="";e==="categories"?(a=Array(n).fill('<li class="category-skeleton skeleton-shimmer"></li>').join(""),i.innerHTML=`<ul class="categories-grid">${a}</ul>`):(a=Array(n).fill(`
    ...
    `).join(""),i.innerHTML=`<ul class="exercises-grid">${a}</ul>`)},R=async(e,t,n="pagination-container")=>{const i=document.getElementById(n);if(!i)return;if(t<=1){i.innerHTML="";return}if(!i.querySelector(".pagination")){const l=await y("pagination");i.innerHTML=l}const a=i.querySelector(".pagination-first"),s=i.querySelector(".pagination-prev"),r=i.querySelector(".pagination-next"),c=i.querySelector(".pagination-last"),d=i.querySelector(".pagination-nbrs");a.disabled=e===1,s.disabled=e===1,s.dataset.page=e-1,r.disabled=e===t,r.dataset.page=e+1,c.disabled=e===t,c.dataset.page=t;const u=we(e,t).map(l=>l==="..."?'<span class="pagination-dots">...</span>':`<button class="pagination-number ${l===e?"pagination-number--active":""}" data-page="${l}" ${l===e?"disabled":""}>${l}</button>`).join("");d.innerHTML=u};function we(e,t){const n=[];if(t<=3){for(let s=1;s<=t;s++)n.push(s);return n}let i,a;e===1?(i=1,a=3):e===t?(i=t-2,a=t):(i=e-1,a=e+1),i>1&&n.push("...");for(let s=i;s<=a;s++)n.push(s);return a<t&&n.push("..."),n}function _e(e,t="pagination-container"){const n=document.getElementById(t);n&&n.dataset.listenerAttached!=="true"&&(n.dataset.listenerAttached="true",n.addEventListener("click",i=>{const a=i.target.closest(".pagination-number, .pagination-btn");if(!a||a.nodeName!=="BUTTON"&&!a.classList.contains("pagination-number")||a.disabled||a.classList.contains("pagination-dots"))return;const s=Number(a.dataset.page);s&&!isNaN(s)&&e(s)}))}function Ee(e="exercises-header"){const t=document.getElementById(e);t?t.scrollIntoView({behavior:"smooth"}):document.querySelector(".exercises-section")?.scrollIntoView({behavior:"smooth"})}const B=e=>{const t=document.getElementById(e);t&&(t.classList.add("active"),t.setAttribute("aria-hidden","false"),document.body.classList.add("modal-open"))},g=e=>{const t=document.getElementById(e);t&&(t.classList.remove("active"),t.setAttribute("aria-hidden","true"),document.body.classList.remove("modal-open"))},M=e=>{if(!e)return;const t=document.getElementById("modal-exercise-gif");t&&(t.src=e.gifUrl||"",t.alt=e.name||"Exercise");const n=document.getElementById("modal-exercise-title");n&&(n.textContent=e.name||"Exercise");const i=document.getElementById("modal-exercise-rating");if(i){const l=e.rating||0,V=Math.floor(l);i.innerHTML=`
      <span class="rating-value">${l.toFixed(1)}</span>
      <div class="rating-stars">
        ${Array.from({length:5},(Qe,J)=>{const q=J<V?"filled":"";return`<svg class="star ${q}" width="18" height="18" viewBox="0 0 18 18" fill="none">
    ...
    `}const a=document.getElementById("modal-target");a&&(a.textContent=e.target||"N/A");const s=document.getElementById("modal-bodypart");s&&(s.textContent=e.bodyPart||"N/A");const r=document.getElementById("modal-equipment");r&&(r.textContent=e.equipment||"N/A");const c=document.getElementById("modal-popular");c&&(c.textContent=e.popularity||"0");const d=document.getElementById("modal-calories");d&&(d.textContent=`${e.burnedCalories||0}/${e.time||0} min`);const m=document.getElementById("modal-description");m&&(m.textContent=e.description||"No description available.");const u=document.getElementById("exercise-modal");u&&(u.dataset.exerciseId=e._id)},P=()=>{g("exercise-modal"),B("rating-modal"),D(),Le()},f=()=>{g("rating-modal"),B("exercise-modal"),D()},D=()=>{const e=document.querySelectorAll("#rating-modal .rating-star"),t=document.getElementById("rating-display-value"),n=document.getElementById("rating-email"),i=document.getElementById("rating-review");e.forEach(a=>a.classList.remove("active")),t&&(t.textContent="0.0"),n&&(n.value="",n.classList.remove("error")),i&&(i.value="")},Le=()=>{const e=document.getElementById("rating-stars"),t=document.getElementById("rating-display-value");e&&e.dataset.listenerAttached!=="true"&&(e.dataset.listenerAttached="true",e.addEventListener("click",n=>{const i=n.target.closest(".rating-star");if(!i)return;const a=parseFloat(i.dataset.rating),s=e.querySelectorAll(".rating-star"),r=Array.from(s).indexOf(i);s.forEach((c,d)=>{d<=r?c.classList.add("active"):c.classList.remove("active")}),t&&(t.textContent=a.toFixed(1))}),e.addEventListener("mouseover",n=>{const i=n.target.closest(".rating-star");if(!i)return;const a=e.querySelectorAll(".rating-star"),s=Array.from(a).indexOf(i);a.forEach((r,c)=>{c<=s?r.classList.add("hover"):r.classList.remove("hover")})}),e.addEventListener("mouseleave",()=>{e.querySelectorAll(".rating-star").forEach(i=>i.classList.remove("hover"))}))},U=()=>{const e=document.getElementById("rating-display-value");return e?parseFloat(e.textContent):0},T="quote-of-the-day";function W(){return new Date().toISOString().split("T")[0]}function Be(){try{const e=localStorage.getItem(T);if(!e)return null;const{quote:t,author:n,date:i}=JSON.parse(e),a=W();return i===a?{quote:t,author:n}:(localStorage.removeItem(T),null)}catch(e){return console.error("Error reading cached quote:",e),null}}function ke(e,t){try{const n={quote:e,author:t,date:W()};localStorage.setItem(T,JSON.stringify(n))}catch(n){console.error("Error caching quote:",n)}}async function z(){try{let e=Be();e||(e=await ce(),ke(e.quote,e.author)),ye(e)}catch(e){console.error("Failed to initialize quote:",e)}}const S="favoriteExercises";function k(){try{const e=localStorage.getItem(S);return e?JSON.parse(e):[]}catch(e){return console.error("Failed to get favorites:",e),[]}}function Ie(e){try{const t=k();return t.some(n=>n._id===e._id)?!1:(t.push(e),localStorage.setItem(S,JSON.stringify(t)),!0)}catch(t){return console.error("Failed to add favorite:",t),!1}}function Q(e){try{const n=k().filter(i=>i._id!==e);return localStorage.setItem(S,JSON.stringify(n)),!0}catch(t){return console.error("Failed to remove favorite:",t),!1}}function _(e){return k().some(n=>n._id===e)}async function Fe(){const e=document.getElementById("favorites-container");if(!e)return;const t=await y("favorites-empty");e.innerHTML=t}async function j(){const e=document.getElementById("favorites-container");if(!e)return;const t=k();if(t.length===0){await Fe();return}const n=await y("favorite-card"),i=t.map(a=>C(n,{id:a._id,name:a.name,burnedCalories:a.burnedCalories||0,bodyPart:a.bodyPart||"N/A",target:a.target||"N/A"})).join("");e.innerHTML=`<ul class="favorites-grid">${i}</ul>`}function Te(e){const t=document.getElementById("modal-close-btn");t&&(t.onclick=()=>g("exercise-modal"));const n=document.getElementById("give-rating-btn");n&&(n.onclick=()=>{P(),Ce(e)});const i=document.getElementById("add-to-favorites-btn");i&&(i.onclick=async()=>{_(e)&&(Q(e),g("exercise-modal"),await j())},_(e)&&(i.textContent="Remove from favorites"));const a=document.getElementById("exercise-modal");a&&(a.onclick=r=>{r.target===a&&g("exercise-modal")});const s=r=>{r.key==="Escape"&&(g("exercise-modal"),document.removeEventListener("keydown",s))};document.addEventListener("keydown",s)}function Ce(e){const t=document.getElementById("rating-modal-close-btn");t&&(t.onclick=()=>f());const n=document.getElementById("rating-form");n&&(n.onsubmit=async s=>{s.preventDefault();const r=U(),c=document.getElementById("rating-email"),d=document.getElementById("rating-review"),m=c?.value.trim(),u=d?.value.trim()||"";if(r<=0){alert("Please select a rating");return}if(!m){alert("Please enter your email"),c?.focus();return}try{await $(e,r,m,u),alert("Rating submitted successfully!"),f()}catch(l){console.error("Failed to submit rating:",l),l.message?.includes("409")?alert("This email has already been used to rate this exercise"):alert("Failed to submit rating. Please try again.")}});const i=document.getElementById("rating-modal");i&&(i.onclick=s=>{s.target===i&&f()});const a=s=>{s.key==="Escape"&&(f(),document.removeEventListener("keydown",a))};document.addEventListener("keydown",a)}async function Me(){const e=document.querySelector(".favorites-page");try{await z(),await j(),Se()}catch(t){console.error("Error initializing favorites page:",t)}finally{e&&e.classList.add("loaded")}}function Se(){const e=document.getElementById("favorites-container");e&&e.dataset.listenerAttached!=="true"&&(e.dataset.listenerAttached="true",e.addEventListener("click",async t=>{const n=t.target.closest(".favorite-delete-btn");if(n){t.stopPropagation();const a=n.dataset.id;if(a){Q(a);try{await j()}catch(s){console.error("Failed to render favorites:",s)}}return}const i=t.target.closest(".favorite-start-btn");if(i){t.stopPropagation();const a=i.dataset.id;if(!a)return;try{const s=await L(a);M(s),B("exercise-modal"),Te(a)}catch(s){console.error("Failed to fetch exercise details:",s),alert("Failed to load exercise details.")}}}))}function je(e){return/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e)}const o={view:"categories",filter:"Muscles",category:null,categoryFilter:null,keyword:"",page:1},E=()=>window.innerWidth<768?9:o.view==="categories"?12:10;async function N(){const e=document.querySelector(".main-content"),t=E();O(o.view,"exercises-container",t);try{await z(),await v()}catch(n){console.error("Error initializing home page:",n)}finally{e&&e.classList.add("loaded")}He(),Re(),_e(Ae),qe()}function qe(){let e,t=E();window.addEventListener("resize",()=>{clearTimeout(e),e=setTimeout(()=>{const n=E();n!==t&&(t=n,o.page=1,v())},300)})}async function v(){const e=document.getElementById("exercises-container");try{const t=E();if(O(o.view,"exercises-container",t),o.view==="categories"){const n=await le({filter:o.filter,page:o.page,limit:t});xe(n.results,"exercises-container"),R(n.page?Number(n.page):1,n.totalPages||1)}else{const n={limit:t,page:o.page};o.categoryFilter==="Muscles"?n.muscles=o.category.toLowerCase():o.categoryFilter==="Body parts"?n.bodypart=o.category.toLowerCase():o.categoryFilter==="Equipment"&&(n.equipment=o.category.toLowerCase()),o.keyword&&(n.keyword=o.keyword);const i=await de(n);be(i.results,"exercises-container"),R(i.page?Number(i.page):1,i.totalPages||1)}}catch(t){console.error("Fetch error:",t),e&&(e.innerHTML='<p class="error-message">Failed to load data. Please try again.</p>')}}function Ae(e){e&&e!==o.page&&(o.page=e,v(),Ee())}function He(){const e=document.getElementById("filter-tabs");e&&e.dataset.listenerAttached!=="true"&&(e.dataset.listenerAttached="true",e.addEventListener("click",async t=>{const n=t.target.closest(".filter-tab");if(n){document.querySelectorAll(".filter-tab").forEach(i=>i.classList.remove("active")),n.classList.add("active"),o.filter=n.dataset.filter,o.view="categories",o.page=1,o.keyword="",o.category=null,$e();try{await v()}catch(i){console.error("Failed to fetch data:",i)}}}))}function Re(){const e=document.getElementById("exercises-container");e&&e.dataset.listenerAttached!=="true"&&(e.dataset.listenerAttached="true",e.addEventListener("click",async t=>{const n=t.target.closest(".category-card");if(n){const a=n.dataset.name,s=n.dataset.filter;if(!a)return;o.view="exercises",o.category=a,o.categoryFilter=s,o.page=1,o.keyword="",Ne(a),Oe();try{await v()}catch(r){console.error("Failed to fetch exercises:",r)}return}const i=t.target.closest(".exercise-start-btn");if(i){const a=i.dataset.id;if(!a)return;try{const s=await L(a);M(s),B("exercise-modal"),Pe(a)}catch(s){console.error(`Failed to fetch exercise details for ${a}:`,s),alert("Failed to load exercise details. Please try again.")}}}))}function Ne(e){const t=document.getElementById("section-title"),n=document.getElementById("exercise-search-form");t&&(t.innerHTML=`Exercises / <span class="category-name">${e}</span>`),n&&n.classList.remove("hidden")}function $e(){const e=document.getElementById("section-title"),t=document.getElementById("exercise-search-form"),n=document.getElementById("exercise-search-input"),i=document.getElementById("exercise-clear-btn");e&&(e.textContent="Exercises"),t&&t.classList.add("hidden"),n&&(n.value=""),i&&i.classList.add("hidden")}function Oe(){const e=document.getElementById("exercise-search-form"),t=document.getElementById("exercise-search-input"),n=document.getElementById("exercise-clear-btn");!e||!t||e.dataset.listenerAttached!=="true"&&(e.dataset.listenerAttached="true",t.addEventListener("input",()=>{t.value.trim()?n.classList.remove("hidden"):n.classList.add("hidden")}),n.addEventListener("click",async()=>{t.value="",n.classList.add("hidden"),t.focus(),o.keyword="",o.page=1;try{await v()}catch(i){console.error("Failed to fetch exercises:",i)}}),e.addEventListener("submit",async i=>{i.preventDefault();const a=t.value.trim();o.keyword=a,o.page=1;try{await v()}catch(s){console.error("Failed to search exercises:",s)}}))}function Pe(e){const t=document.getElementById("modal-close-btn");t&&(t.onclick=()=>g("exercise-modal"));const n=document.getElementById("give-rating-btn");n&&(n.onclick=()=>{P(),De(e)});const i=document.getElementById("add-to-favorites-btn");i&&(i.onclick=async()=>{if(_(e))alert("This exercise is already in your favorites");else{const r=await L(e);Ie(r),i.innerHTML=`
          Added to favorites
        ...
        `,setTimeout(()=>{i.textContent="Add to favorites"},2e3)}},_(e)&&(i.textContent="Already in favorites",i.disabled=!0,i.style.opacity="0.6"));const a=document.getElementById("exercise-modal");a&&(a.onclick=r=>{r.target===a&&g("exercise-modal")});const s=r=>{r.key==="Escape"&&(g("exercise-modal"),document.removeEventListener("keydown",s))};document.addEventListener("keydown",s)}function De(e){const t=document.getElementById("rating-modal-close-btn");t&&(t.onclick=()=>f());const n=document.getElementById("rating-form");n&&(n.onsubmit=async s=>{s.preventDefault();const r=U(),c=document.getElementById("rating-email"),d=document.getElementById("rating-review"),m=c?.value.trim(),u=d?.value.trim()||"";if(r<=0){alert("Please select a rating");return}if(!m){alert("Please enter your email"),c?.focus();return}if(!je(m)){alert("Please enter a valid email address"),c?.classList.add("error"),c?.focus();return}c?.classList.remove("error");try{await $(e,r,m,u),alert("Rating submitted successfully!"),f();const l=await L(e);M(l)}catch(l){console.error("Failed to submit rating:",l),l.message?.includes("409")?alert("This email has already been used to rate this exercise"):alert("Failed to submit rating. Please try again.")}});const i=document.getElementById("rating-modal");i&&(i.onclick=s=>{s.target===i&&f()});const a=s=>{s.key==="Escape"&&(f(),document.removeEventListener("keydown",a))};document.addEventListener("keydown",a)}function h(e){const t=document.getElementById("nav-home"),n=document.getElementById("nav-favorites");!t||!n||(t.classList.remove("active"),n.classList.remove("active"),e==="/favorites"?n.classList.add("active"):t.classList.add("active"))}function Ue(){const e=document.querySelector(".nav");e&&e.addEventListener("click",n=>{const i=n.target.closest(".nav-link");if(!i)return;n.preventDefault();const a=i.dataset.route;a&&(w.navigate(a),h(a))});const t=document.querySelector(".mobile-nav");t&&t.addEventListener("click",n=>{const i=n.target.closest(".mobile-nav-link");if(!i)return;n.preventDefault();const a=i.dataset.route;a&&(w.navigate(a),h(a),x())})}function We(){const e=document.getElementById("burger-btn"),t=document.getElementById("mobile-menu"),n=document.getElementById("mobile-close-btn");!e||!t||!n||(e.addEventListener("click",ze),n.addEventListener("click",x),t.addEventListener("click",i=>{i.target===t&&x()}),document.addEventListener("keydown",i=>{i.key==="Escape"&&t.classList.contains("is-open")&&x()}))}function ze(){const e=document.getElementById("mobile-menu"),t=document.getElementById("burger-btn");e&&(e.classList.add("is-open"),document.body.style.overflow="hidden"),t&&t.setAttribute("aria-expanded","true")}function x(){const e=document.getElementById("mobile-menu"),t=document.getElementById("burger-btn");e&&(e.classList.remove("is-open"),document.body.style.overflow=""),t&&t.setAttribute("aria-expanded","false")}document.addEventListener("DOMContentLoaded",()=>{Ue(),We()});w.on("/",async()=>{await I("home"),await N(),F(),h("/")}).on("/favorites",async()=>{await I("favorites"),await Me(),F(),h("/favorites")}).notFound(async()=>{await I("home"),await N(),F(),h("/")});function F(){const e=document.getElementById("subscription-form");e&&e.addEventListener("submit",async t=>{t.preventDefault();const n=e.email.value;try{await me(n),alert("Successfully subscribed!"),e.reset()}catch{alert("Subscription failed. Please check your email or try again later.")}})}document.addEventListener("DOMContentLoaded",()=>{w.resolve()});
